...
process_sequence () {
    ...
    if [ -f ${base_dir}/vicon_s_delay.txt ]; then
        ...
        output_path="/home/schiavazza/data/hpe/vicon_dataset/yarp/zhichao/${seq_name}_s/"
        echo "Saving cdf to: ${output_path}"

        if [ -f $output_path ]; then
            echo -e "\t\tALREADY DONE" >> $status_file
        else
            python3 ../export_yarp.py --dvs_path ${input_static} --vicon_path ${c3d_file} --intrinsic ${c3d_base_dir}/calib-s.txt --extrinsic ${c3d_base_dir}/extrinsic_s.npy --output_path ${output_path} --labels ../config/labels_joints.yml --vicon_delay ${delay} --no_camera_markers --subject $subject
            s=$?
            if [ $s -ne 0 ]; then
    ...
    if [ -f ${base_dir}/vicon_d_delay.txt ]; then
        ...
        echo "Delay d: ${delay}"
        echo -e "\t\t${delay} " >> $status_file
        output_path="/home/schiavazza/data/hpe/vicon_dataset/yarp/zhichao/${seq_name}_d/"
        echo "Saving cdf to: ${output_path}"
        if [ -f $output_path ]; then
            echo -e "\t\tALREADY DONE" >> $status_file
        else
            python3 ../export_yarp.py --dvs_path ${input_dynamic} --vicon_path ${c3d_file} --intrinsic ${c3d_base_dir}/calib-d.txt --extrinsic ${c3d_base_dir}/extrinsic_d.npy --output_path ${output_path} --labels ../config/labels_joints.yml --vicon_delay ${delay} --subject $subject
            s=$?
            if [ $s -ne 0 ]; then
                echo -e "\t\tFAIL " >> $status_file
                failed=$((failed + 1))
